function CGame(oData) {
    ...
    this._initHitArea = function() {
        ...
        _aHitAreaListeners[0] = _oHitArea.on("mousedown", function(evt) {
            oParent.onPressed(evt);
        });
        _aHitAreaListeners[1] = _oHitArea.on("pressmove", function(evt) {
    ...
    this._resetStage = function() {
        ...
        createjs.Tween.get(oFade)
            .to({
                alpha: 0
            }, RESET_STAGE_FADE_SPEED, createjs.Ease.cubicOut)
            .call(function() {
    ...
    this.addCoin = function() {
        ...
        createjs.Tween.get(oCoin)
            .to({
                x: oIconPos.x
            }, COIN_SPEED, createjs.Ease.quintIn);
        createjs.Tween.get(oCoin)
            .to({
                y: oIconPos.y
            }, COIN_SPEED, createjs.Ease.cercOut)
            .call(function() {
    ...
    this.initNewLevelText = function() {
        ...
        createjs.Tween.get(oLevelTextBack)
            .to({
                y: 0,
                alpha: 0
            }, TEXT_MESSAGES_SPEED * 3, createjs.Ease.sineOut)
            .call(function() {
    ...
    this._gameOver = function(szText) {
        ...
        if (_oEndPanel === null) {
            ...
            setTimeout(function() {
                _oEndPanel = new CEndPanel(s_iTotalScore, szText);
            }, MESSAGE_GAMEOVER_DELAY);
            _bDisableEvents = true;
        }
    };
    ...
    this._removeTutorial = function() {
        createjs.Tween.get(_oTutorialContainer)
            .to({
                alpha: 0
            }, TUTORIAL_FADEOUT_SPEED, createjs.Ease.quadIn)
            .call(function() {
                createjs.Tween.removeTweens(_oTutorialContainer);
                _oGameContainer.removeChild(_oTutorialContainer);
            });
    ...
    this.onFlipCompleted = function() {
        ...
        createjs.Tween.get(_oFlipCompletedTextBack)
            .to({
                y: 0,
                alpha: 0
            }, TEXT_MESSAGES_SPEED * 2, createjs.Ease.sineOut)
            .call(function() {
    ...
    this._addCollisionAnimation = function(oBodyPart) {
        ...
        oCollisionAnim.addEventListener("animationend", function() {
            _oObjectsContainer.removeChild(oCollisionAnim);
        });
    ...
    this._updateCamera = function() {
        ...
        createjs.Tween.get(_oObjectsContainer).to({
            y: iDestY
        }, CAMERA_MOVEMENT_SPEED, createjs.Ease.cubicOut);

        // UPDATE THE DEBUG CANVAS POSITION            
        if (DEBUG_BOX2D) {
            var oCanvasOffset = {
                x: 0,
                y: _oObjectsContainer.y
            };
