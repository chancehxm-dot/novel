...
logo.showing = true;
logo.hiding = false;
logo.img = loadGameImage('./src/wav/logo.png'); // 626 x 144 ->300x69
logo.img.style.position = 'absolute';
logo.img.style.opacity = 0;
logo.img.style.zIndex = '42069';
logo.reposition = function() {
    logo.img.style.left = Math.floor((ww - 300) / 2) + 'px';
    logo.img.style.top = Math.floor(Math.cos(logo.fr / 9) * 20 + (hh - 69 - 88) / 2) + 'px';
}
...
gameover.showing = false;
gameover.hiding = true;
gameover.img = loadGameImage('./src/wav/gameover.png'); // 626 x 144 ->300x69
...
gameover.reposition = function() {
    gameover.img.style.left = Math.floor((ww - 300) / 2) + 'px';
    gameover.img.style.top = Math.floor(Math.cos(gameover.fr / 32) * 4 + (hh - 200 - 69 - 88) / 2) + 'px';
}
...
fbshare.reposition = function() {
    fbshare.img.style.left = Math.floor((ww - 160) / 2) + 'px';
    fbshare.img.style.top = Math.floor(Math.cos(fbshare.fr / 12) * 5 + (hh + 95 + 250 - 88) / 2) + 'px';
}
...
var playagain = {}; //  252x71     256x152 -> 200x56
playagain.a = -.2;
playagain.fr = 0;
playagain.showing = false;
playagain.hiding = true;
...
playagain.reposition = function() {
    playagain.img.style.left = Math.floor((ww - 200) / 2) + 'px';
    playagain.img.style.top = Math.floor(Math.cos(playagain.fr / 14) * 5 + (hh + 100 + 71 - 88) / 2) + 'px';
}
...
c2s.showing = true;
c2s.hiding = false;
c2s.img = loadGameImage('./src/wav/clicktostart.png'); // 337 x 75 -> 200x45
c2s.img.style.position = 'absolute';
c2s.img.style.opacity = 0;
c2s.img.style.zIndex = '42070';
c2s.reposition = function() {
    c2s.img.style.left = Math.floor((ww - 200) / 2) + 'px';
    c2s.img.style.top = Math.floor((hh + 200) / 2) + 'px';
}
...
parody.div.innerHTML = '<a href="" target="_blank"><img src="logo.png" height="24px"></a>';
