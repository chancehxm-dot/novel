...
GAME.Crossbow = function(container, positionY) {
    ...
    this.collisionBox = {
        x: this.flechaBallesta.position.x - this.arcoBallesta.width * 0.5,
...
GAME.Crossbow.prototype.shoot = function(location) {
    ...
    if (gameEngine.isMobile.any()) {
        ...
        TweenLite.to(this.arcoBallesta.position, 0.3, {
            x: posX + Math.cos(angle) * distanceMin,
            y: posY + Math.sin(angle) * distanceMin,
        ...
        TweenLite.to(this.arcoBallesta.position, 0.3, {
            x: posX + Math.cos(angle + Math.PI) * distanceMin,
            y: posY + Math.sin(angle + Math.PI) * distanceMin,
            ease: Elastic.easeOut,
            overwrite: "all"
        });
    }
...
GAME.Crossbow.prototype.specialAppear = function() {
    ...
    TweenLite.to(this.flechaBallestaBlanco, 2, {
        alpha: 0.0,
        ease: Sine.easeIn,
        overwrite: "all"
    });
