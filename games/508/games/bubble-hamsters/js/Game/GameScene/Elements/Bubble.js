...
GAME.Bubble.prototype.inMovement = function(positionX, positionY) {
    if (!this.movingNewRow) {
        ...
        this.tweenMovement = TweenLite.to(this.sprite.position, 0.3, {
            x: posX,
            y: posY,
            ease: Back.easeOut,
            overwrite: "all"
...
GAME.Bubble.prototype.newRow = function(durationMovement) {
    ...
    if (this.tweenMovement) this.tweenMovement.pause();
    if (durationMovement < 0.4) TweenLite.to(this.sprite.position, durationMovement, {
        x: x,
        y: y,
        ease: Sine.easeInOut,
        overwrite: "all",
        onCompleteScope: this,
        onComplete: this.movingNewRowFinish
    });
    else TweenLite.to(this.sprite.position, durationMovement, {
        x: x,
        y: y,
...
GAME.Bubble.prototype.moveToNewPosition = function() {
    ...
    if (this.tweenMovement) this.tweenMovement.pause();
    this.tweenMovement = TweenLite.to(this.sprite.position, 0.4, {
        x: x,
        y: y,
        ease: Back.easeInOut,
        overwrite: "all"
...
GAME.Bubble.prototype.explode = function(notify) {
    TweenLite.to(this.sprite, 0.15, {
        alpha: 0,
        ease: Circ.easeOut,
        overwrite: "all",
        delay: this.branchDepth * 0.05 + 0.05
    });
    TweenLite.to(this.sprite.scale, 0.15, {
        x: 1.2,
        y: 1.2,
        ease: Circ.easeOut,
        overwrite: "all",
        delay: this.branchDepth * 0.05 + 0.05,
        onStart: this.setExploding,
        onStartScope: this,
        onStartParams: [notify]
    });
}
...
GAME.Bubble.prototype.showScore = function(x, y, score) {
    if (this.bubbleType == BUBBLE_TYPE.CYAN) this.scoreText.setStyle({
        font: "GameScene_CyanFont",
        align: "left"
    });
    else if (this.bubbleType == BUBBLE_TYPE.RED) this.scoreText.setStyle({
        font: "GameScene_RedFont",
        align: "left"
    });
    else if (this.bubbleType == BUBBLE_TYPE.BLUE) this.scoreText.setStyle({
        font: "GameScene_BlueFont",
        align: "left"
    });
    else if (this.bubbleType == BUBBLE_TYPE.GREEN) this.scoreText.setStyle({
        font: "GameScene_GreenFont",
        align: "left"
    });
    else if (this.bubbleType == BUBBLE_TYPE.YELLOW) this.scoreText.setStyle({
        font: "GameScene_YellowFont",
        align: "left"
    });
    else if (this.bubbleType == BUBBLE_TYPE.ORANGE) this.scoreText.setStyle({
        font: "GameScene_OrangeFont",
        align: "left"
    });
    else if (this.bubbleType == BUBBLE_TYPE.PURPLE) this.scoreText.setStyle({
        font: "GameScene_MagentaFont",
        align: "left"
    });
    ...
    TweenLite.to(this.scoreText, 0.1, {
        alpha: 1.0,
        ease: Sine.easeOut,
        overwrite: "all"
    });
    TweenLite.to(this.scoreText.position, 0.1, {
        y: y - this.scoreText.textHeight * 0.5,
        ease: Sine.easeOut,
        overwrite: "all",
        onCompleteScope: this,
        onComplete: this.hideScore
    });
}
...
GAME.Bubble.prototype.hideScore = function() {
    ...
    TweenLite.to(this.scoreText.position, 0.1, {
        y: this.scoreText.position.y - this.scoreText.textHeight * 0.5,
        delay: 1.2,
        ease: Sine.easeOut,
        overwrite: "all"
    });
...
GAME.Bubble.prototype.update = function(dt) {
    if (this.state == BUBBLE_STATE.MOVING) {
        ...
        if (this.seedSprite.position.y > defaultHeight * 0.5 && !this.disappearSeed) {
            ...
            TweenLite.to(this.seedSprite, 0.2, {
                alpha: 0,
                ease: Sine.easeIn,
                overwrite: "all",
